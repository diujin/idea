...
//routes
app.get('/idea/client/:build', async (req, res) => {
...
app.get('/idea/', (req, res) => {
  const props = { title: 'Idea Documentation' };
...
app.get('/idea/**', (req, res) => {
  ...
  const route = (() => {
    const route = req.path.endsWith('.html')
      ? req.path.substring(6, req.path.length - 5)
      : req.path.substring(6);
  ...
  //else if static file
  const resource = (req.url || '').substring(6).replace(/\/\//, '/'); 
